...
export default async function PublicCreatorPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const [isFollowing, creatorPlan, subscriberAccess, premiumAccess] = await Promise.all([
  ...
  const visible = creator.contents.filter((content) => content.visibility === "PUBLIC" || content.visibility === "FOLLOWERS" && isFollowing || content.visibility === "SUBSCRIBERS" && (subscriberAccess || premiumAccess) || user?.id === creator.userId || user?.role === "ADMIN");
  return <><Header /><main><section className="creator-profile"><div className="creator-header"><div className="large-avatar">{creator.avatarUrl ? <img src={creator.avatarUrl} alt={creator.displayName}/> : <span>{creator.displayName.charAt(0).toUpperCase()}</span>}</div><div><div className="verified-line"><h1>{creator.displayName}</h1><span className="badge">✓ Verificado</span></div><p className="muted">@{creator.username}</p><p className="muted">{creator._count.followers} seguidores · {creator._count.subscriptions} assinantes</p>{user && user.id !== creator.userId ? <CreatorFollowButton creatorId={creator.id} isFollowing={isFollowing}/> : null}</div></div>
    <div className="card"><h2>Sobre</h2><p className="muted">{creator.bio ?? "Este criador ainda não adicionou uma biografia."}</p></div>
    <div className="card"><h2>Conteúdo</h2>{visible.length === 0 ? <div className="empty-state">Nenhum conteúdo disponível para o seu nível de acesso.</div> : <div className="creator-content-grid">{visible.map((content) => <article key={content.id}><span className="badge">{content.visibility}</span><h3>{content.title}</h3>{content.mediaType === "VIDEO" ? <video className="content-media" src={`/api/media/${content.id}`} controls preload="metadata"/> : <img className="content-media" src={`/api/media/${content.id}`} alt={content.title}/>}</article>)}</div>}</div>
